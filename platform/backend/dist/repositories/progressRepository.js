...
class ProgressRepository {
    async recordAttemptAndUpdateProgress(input) {
        ...
        try {
            logger_1.logger.info("db.progress.record_attempt.start", { userId: input.userId, lessonId: input.lessonId });
            ...
            const upsertResult = await databaseClient.query(QUERY_UPSERT_PROGRESS, [
                input.userId,
                input.lessonId,
                input.isCorrect,
                learningConstants_1.LEARNING_CONSTANTS.completionTargetPercent,
            ]);
            await databaseClient.query("COMMIT");
            logger_1.logger.info("db.progress.record_attempt.success", {
                userId: input.userId,
            ...
            logger_1.logger.error("db.progress.record_attempt.error", error, { userId: input.userId, lessonId: input.lessonId });
            throw new errorHandler_1.HttpError(500, learningConstants_1.LEARNING_ERROR_MESSAGES.failedToSaveAttempt);
        }
        finally {
    ...
    async getProgressByUserId(userId) {
        try {
            logger_1.logger.info("db.progress.get_by_user_id.start", { userId });
            const queryResult = await db_1.dbPool.query(QUERY_SELECT_PROGRESS_BY_USER, [userId]);
            logger_1.logger.info("db.progress.get_by_user_id.success", { userId, rowCount: queryResult.rowCount });
            return queryResult.rows;
        }
        catch (error) {
            logger_1.logger.error("db.progress.get_by_user_id.error", error, { userId });
            throw new errorHandler_1.HttpError(500, learningConstants_1.LEARNING_ERROR_MESSAGES.failedToReadProgress);
        }
    }
}
